...
<script type="text/javascript">
    ...
    function  asyncRequesyData(pageno) {
        ...
        $.ajax({
            type : "POST",
            url : "${ctx}/serviceman/proc/asyncRequestData",
            data : jsonData,
            beforeSend : function() {
            ...
            success : function(result) {
                ...
                if (result.success) {
                    ...
                    var procs = projectPage.datas; // 用户
                    $.each(procs,function(index, proc) {
                        tableContent += '<tr id=" '+ proc.id + '">';
                        tableContent += '	<td>' + (index + 1) + '</td>';
                        tableContent += '	<td><input type="checkbox" name="procid" value="'+proc.id+'"></td>';
                        tableContent += '    <td>' + proc.name + '</td>';
                        tableContent += '    <td>' + proc.version + '</td>';
                        tableContent += '    <td>' + "测试用"+ '</td>';
                        tableContent += '    <td>';
    ...
    function del(id,name){
        layer.confirm("是否删除流程【" + name + "】信息?", {
            ...
            $.ajax({
                type : "POST",
                url : "${ctx}/serviceman/proc/delete",
                data : {
                    id : id
                },
                success : function(result) {
    ...
    function delBatch(){
        // 选中的checkbox
        var boxes = $("#procData :checkbox:checked");
        if (boxes.length == 0) {
            layer.msg("请选择删除的分类", {
                time : 2000,
                icon : 5,
            ...
            layer.confirm("是否删除选中分类信息?", {
                ...
                $.ajax({
                    type : "POST",
                    url : "${ctx}/serviceman/proc/deleteBatch",
                    data : $("#procForm").serialize(), // 元素属性必须使用name
                    success : function(result) {
                        if (result.success) {
                            layer.msg("删除成功",{time:2000,icon:6},function(){
                                // 回调函数做页面跳转,跳转到列表
    ...
    function addModal(){
        $(".modal-title").empty().text("部署流程");
        var body = '<div class="form-group">'
                    +'<form id="proc_form" method="post" enctype="multipart/form-data"> '
        ...
        var footer = '<button type="button" id="insertBtn" class="btn btn-success">'
            +'<i class="glyphicon glyphicon-plus"></i> 部署'
            +'</button>'
    ...
    function editModal(id,name){
        $(".modal-title").empty().text("流程详细信息查看");
        var body = '<div class="form-group">'+
            '<label for="exampleInputPassword1">流程名称</label>' +
