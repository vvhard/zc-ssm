...
<script type="text/javascript">
    $(function () {
        ...
        // 列表展开
        $("a[href='${ctx}/serviceman/cert/index']").css("color", "red");
        //加上tree close样式
        $("a[href='${ctx}/serviceman/cert/index']").parents(".list-group-item")
            .removeClass("tree-closed");
        $("a[href='${ctx}/serviceman/cert/index']").parent().parent("ul").show(100);
    ...
    function  asyncRequesyData(pageno) {
        ...
        $.ajax({
            type : "POST",
            url : "${ctx}/serviceman/cert/asyncRequestData",
            data : jsonData,
            beforeSend : function() {
            ...
            success : function(result) {
                ...
                if (result.success) {
                    ...
                    var certs = certPage.datas; // 用户
                    $.each(certs,function(index, cert) {
                        tableContent += '<tr id=" '+ cert.id + '">';
    ...
    function del(certid,name){
        layer.confirm("是否删除资质【" + name + "】信息?", {
            icon : 3,
            title : '提示'
        }, function(cindex) {
            // 删除信息
            $.ajax({
                type : "POST",
                url : "${ctx}/serviceman/cert/delete",
                data : {
                    id : certid
                },
                success : function(result) {
